...
public class LogbackConfig extends BasicConfigurator implements Configurator {
  ...
  private RollingFileAppender<ILoggingEvent> getFileAppender(LoggerContext lc) {
    ...
    String file = String.format("%s/demo-errors.txt", logDir);
    rfa.setFile(file);
    TimeBasedRollingPolicy<ILoggingEvent> policy = new TimeBasedRollingPolicy<>();
    String namePattern = String.format("%s/demo-errors.txt.%%d{yyyy-MM-dd}.zip ", logDir);
  ...
  public void configure(LoggerContext lc) {
    ...
    Logger appLogger = lc.getLogger("com.demo");

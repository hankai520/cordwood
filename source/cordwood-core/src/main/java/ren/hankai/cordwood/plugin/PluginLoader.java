...
public interface PluginLoader {
    ...
    /**
     * 插件基包。例如：所有插件都在 com.example.demo 包中，则基包为 com.example.demo、或
     * com.example 或 com。多个基包用逗号分隔。
    ...
    /**
     * 对于基于spring框架的插件包，在 jar 文件的 META-INF/MANIFEST.MF 文件中需要加入以下配置来告知加载器
     * 应该使用哪个配置类作为插件的 spring 配置类。加载器将会使用此类的配置来初始化插件包专用的 spring 上下文。
     */

/**
 * 应用引擎，进行各种必要初始化工作、提供必要工具方法和类。
 * */
function AppEngine() {
  var collapsed = false; //边栏菜单是否已收起
  var is_mobile = false; //当前屏幕尺寸是否是移动终端尺寸
  var is_mini_menu = false; //当前是否已启用迷你菜单
  var is_fixed_header = false; //当前页面头部是否是固定的
  var responsiveFunctions = []; //响应式函数数组，用于自定义在屏幕尺寸发生变化时需要调用的函数
  //执行响应式函数
  this.runResponsiveFunctions = function() {
  ...
  //初始化响应式边栏菜单
  this.responsiveSidebar = function () {
...
var App = function () {
  ...
  var handleProgress = function () {
    $(document).ready(function(){
          ...
          function() {
            ...
            var vInterval = setInterval(
              function(){
                ...
                var iElapsedMs = new Date() - aOpts.start, // elapsed time in
                                                            // MS
                  iDays = parseInt(iLeftMs / iDms), // elapsed days
                  iHours = parseInt((iLeftMs - (iDays * iDms)) / iHms), // elapsed
                                                                        // hours
                  iMin = parseInt((iLeftMs - (iDays * iDms) - (iHours * iHms)) / iMms), // elapsed
                                                                                        // minutes
                  iSec = parseInt((iLeftMs - (iDays * iDms) - (iMin * iMms) - (iHours * iHms)) / iCms), // elapsed
                                                                                                        // seconds
      ...
      var iNow = new Date().setTime(new Date().getTime() + 5 * 1000); // now
                                                                      // plus 5
                                                                      // secs
  ...
  var handleNestableLists = function () {
    var updateOutput = function(e)
    {
      ...
      if (window.JSON) {
        output.val(window.JSON.stringify(list.nestable('serialize')));// , null,
                                                                      // 2));
      } else {
  ...
  var handleDataTables = function () {
    ...
    $('.datatable').each(function(){
      var datatable = $(this);
      // SEARCH - Add the placeholder for Search and Turn this into in-line form
      // control
    ...
    /* Loading Remote Data */
        ...
        ajax: { // instead of writing the function to execute the request we use
          ...
          return {
            q: term, // search term
            page_limit: 10,
            apikey: "uekzdmffsrmqzwdtcgmc5yu9" // please do not copy API. Use
                                                // your own. Copying will be
          ...
          // preselected movie's id
          // this function resolves that id attribute to an object that select2
          // can render
          // using its formatResult renderer - that way the movie name is shown
          // preselected
          var id=$(element).val();
          if (id!=="") {
          ...
          dropdownCssClass: "bigdrop", // apply css that makes the dropdown
                                        // taller
          escapeMarkup: function (m) { return m; } // we do not want to escape
                                                    // markup since we are
                                                    // displaying html in
                                                    // results
      });
    ...
    function chart6() {
      var errorBar = {
        ...
        var insertionPoint = xChart.visutils.getInsertionPoint(9),
          container,
          // Map each error bar into 3 points, so it's easier to draw as a
          // single path
  ...
  var handleCalendar = function () {
    ...
    var calendar = $('#calendar').fullCalendar({
      ...
      droppable: true, // this allows things to be dropped onto the calendar
                        // !!!
      drop: function(date, allDay) { // this function is called when something
                                      // is dropped
  ...
  return {
        ...
        init: function () {
          ...
          handleSidebarAndContentHeight();  // Function to hide sidebar and main
                                            // content height
